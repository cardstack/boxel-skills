...
| 10 | **Missing Boxel CLI lint.** Every `.gts` code change must run the installed npm `@cardstack/boxel-cli` lint gate before being called done. Use `npx boxel file lint` / `npx boxel lint`; the current monorepo CLI has no `npx boxel check`. Clean means `No lint issues found` or JSON with an empty `messages` array. See `references/lint-workflow.md`. |
