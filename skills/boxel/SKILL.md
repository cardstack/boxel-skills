...
| 4 | Missing `fitted` format on a CardDef. All CardDefs need `isolated`, `embedded`, AND `fitted`. |
| 5 | **Query: `filter: { on: ref }` with no predicate. To match all cards of a type, use `filter: { type: ref }`.** `on` is a *scope* for predicates (`eq`/`contains`/`range`), not a filter itself. A bare `{ on: ref }` returns zero rows silently. |
| 6 | **Query sort: custom field without `on: ref`.** Only `lastModified`, `createdAt`, `cardURL` work as sort keys without `on`. Sorting on `lastName`, `dates.start`, anything custom — sort expression MUST include `on: ref` or the query is rejected. |
| 7 | **Query refs: hand-rolled URL or `Symbol.for('realmURL')`.** Use `codeRef(here, path, name)` and import `realmURL` (a Symbol) from `@cardstack/runtime-common`. `Symbol.for('realmURL')` produces a *different* Symbol that doesn't match what the host injected → `realms: []` → query never fires. |
| 8 | **Template helper/modifier import mismatch.** In `.gts`, `fn`, `on`, `concat`, `get`, `array`, `hash`, Boxel UI predicates, and formatters must be imported explicitly. Do **not** use `(perform ...)` in strict-mode templates and do **not** import `ember-concurrency/helpers/perform`. Trigger tasks through a scoped handler like `startSave = () => this.saveTask.perform();` and bind `{{on 'click' this.startSave}}`. Run the import preflight in `references/common-imports.md` before finalizing. |
| 9 | **Missing Boxel CLI lint.** Every `.gts` code change must run the installed npm `@cardstack/boxel-cli` lint gate before being called done. Use `npx boxel file lint` / `npx boxel lint`; the current monorepo CLI has no `npx boxel check`. Clean means `No lint issues found` or JSON with an empty `messages` array. See `references/lint-workflow.md`. |
| 10 | **Inline media/binary in card JSON.** Never persist `data:`, `blob:`, base64, ArrayBuffer text, MP3/image bytes, or generated media payloads in `StringField`, `TextAreaField`, `outputText`, JSON attributes, or `Base64ImageField`. Media assets must be stored as realm files (`linksTo(FileDef/ImageDef/PngDef/etc.)`). For generated bytes, use `WriteBinaryFileCommand` first. |
...
| 15 | **🚨 Base modules import by URL, never by package name.** Card API and base fields come from `https://cardstack.com/base/<module>`: `import { CardDef, field, contains } from 'https://cardstack.com/base/card-api';` and `import StringField from 'https://cardstack.com/base/string';`. `@cardstack/base/...` looks like a normal npm import and is always wrong in a realm — the module never resolves, the card fails to load, and the fix costs a repair turn. Package-style imports that ARE correct: `@cardstack/boxel-ui/*`, `@cardstack/boxel-icons/*`, `@cardstack/runtime-common`, `@ember/*`, `@glimmer/*`, `ember-*`. Full list: `references/common-imports.md`. |
