...
**If the brand demands a specific outer treatment** (sharp corners, custom border), put it on the **Theme card** (`--background`, `--border`; `--radius` reaches the inner `--boxel-border-radius-*` scale, not the wrapper's own corners). The CardContainer reads those tokens directly, so every linked card gets it for free — without format CSS contention.
