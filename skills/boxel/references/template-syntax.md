...
#### ⚠️ `{{#if}}` — don't wrap property access in `(…)`

Glimmer treats `(expr)` inside `{{...}}` as "invoke `expr` as a helper". Use the bare path for property access (including class getters):

```hbs
{{!-- ✅ Correct: direct property access; truthy when count > 0 --}}
{{#if this.activityCount}} ... {{/if}}

{{!-- ❌ Wrong: Glimmer tries to invoke `this.activityCount` as a helper.
     Class getters are NOT helpers, so the subexpression evaluates falsy
     EVEN WHEN the getter returns a truthy number elsewhere in the template. --}}
{{#if (this.activityCount)}} ... {{/if}}
```

The silent-failure trap: another line like `{{this.activityCount}} activities` renders `2 activities` (property access works), but the `(this.activityCount)`-guarded block never enters its true branch. You see the count display, the empty-state copy renders below it, and it looks like the data isn't there.

Use `(...)` ONLY when invoking helpers with arguments: `(eq @model.status 'done')`, `(gt this.count 0)`. Never around a bare property or getter.
