...
> **Naming convention check.** When you see `from '../<libname>'` or `from './<libname>/index.js'`, that's a **realm-bundled library** — the actual code lives inside the realm filesystem next to your card, generated by `scripts/build-realm-bundle.mjs` from upstream source. The specific library catalogue is workspace-specific; see `.claude/extensions/`.
...
Substantial JS libraries that ship as generated bundles inside a realm filesystem (Boxel's de-facto package manager — realms don't resolve npm). The library catalogue, canonical URLs, import statements, and rebuild commands are **workspace-specific** and live in [`.claude/extensions/`](../../extensions/README.md).

Ask the user which realm hosts their `common-libs`-equivalent bundle, and consult the matching extension pattern (e.g. `library-<name>`) for the import block. If the workspace has no extension for the library the user names, fall back to Tier 4 (ESM CDN) or to authoring the bundle from scratch via the realm-bundle-shim pattern in `.claude/extensions/`.

**When the target realm doesn't have the library** — and a snapshot bundle exists in [`.claude/extension-libs/`](../../extension-libs/README.md) — the agent can upload the snapshot subtree (`boxel realm push` for multi-file bundles like `bxl/` and `surfaces/`; `boxel file write` for single-file bundles like `ember-flow-dependencies.js`). The companion extension under `.claude/extensions/library-<name>/` covers the *import statements*; the `extension-libs/<name>/` subtree carries the *bytes*. See the extension-libs README for the full upload workflow.
...
| Any realm-bundled library (UI surface kits, computation runtimes, canvas/flow editors, etc.) | 3 | See `.claude/extensions/` for canonical URLs in this workspace |
...
## Build / rebuild pointers

Tier-3 libraries are built by each library's source repo (`npm run realm` typically) and emitted into a shared `common-libs`-style realm. The library catalogue, source paths, and rebuild commands live in `.claude/extensions/` — they vary per workspace and per organisation. Ask the user which realm hosts the shared bundles in this workspace.
