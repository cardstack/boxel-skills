...
2. **Copy and edit.** Take an existing Theme, `copy-card` it, and edit its structured fields (`rootVariables`, `darkModeVariables`, `typography`, and on a `BrandGuide` the palette and mark fields). `cssVariables` and `cssImports` are computed from those fields, so never edit them directly; stylesheets that cannot be derived from the font stacks go in `customCssImports`. Catalog Themes have a "Copy and Edit" menu item built in.
