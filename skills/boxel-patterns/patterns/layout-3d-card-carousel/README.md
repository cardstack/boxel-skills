...
# layout-3d-card-carousel — Cards on a virtual cylinder via `PrerenderedCardSearch` + CSS perspective

**What this gives you:** A query-driven 3D carousel where each result from a `PrerenderedCardSearch` is positioned on a virtual cylinder around the viewer. Auto-rotate, hover lift, click-to-focus, filter-state reactive — all CSS-and-tracked-state, no library.
...
Pick a flat grid (`CardsGrid`, `prerenderedCardSearchComponent` with `display: grid`) when speed of scanning matters more than impact. The 3D carousel is for the *hero* spot.
...
The query side: `PrerenderedCardSearch` with `@isLive={{true}}` feeds results into the carousel. Add filter state (`@tracked selectedTeam`, etc.) that rebuilds the `query` getter, and the carousel rebuilds with new cards.
...
<div class='stage'>
  <div class='cylinder {{if this.isRotating "spinning"}}'>
    {{#let (component @context.prerenderedCardSearchComponent) as |PrerenderedCardSearch|}}
      <PrerenderedCardSearch @query={{this.query}} @realms={{this.realms}}
                              @format='fitted' @isLive={{true}}>
        <:response as |cards|>
...
- `@isLive={{true}}` cost. Re-fetches on every realm change. Default `false` unless you genuinely need live updates from concurrent edits.
...
- **`@isLive={{true}}` cost.** Re-fetches on every realm change. Flip on only when needed.
...
- **Card chrome breaks immersion.** Children rendered through `PrerenderedCardSearch` come with `CardContainer` chrome (rounded corners, halo). For a clean carousel look, use `@displayContainer={{false}}` per-card OR style the chrome via `:deep(.boxel-card-container)` from scoped CSS. See `boxel-ui-guidelines/references/delegated-render-control.md`.
