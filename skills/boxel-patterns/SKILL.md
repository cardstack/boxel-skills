...
- **`resource-consume-from-context`** — How to read what `@context.getCards` / `getCard` / `getCardCollection` hand back: they are resources, not promises. Hold one as a class field and render straight off `instances` / `card` / `cards`, gated on `isLoading` / `isLoaded` and the error properties. Carries the smell list to check card code against — awaiting a resource, `.then` on one, copying `.instances` into `@tracked`, polling one on a timer, reading one from a constructor. **Read this before writing any code that consumes one of the three getters.**
...
- `integrate-capture-card-format` — `CaptureCardTool` captures a **settled PNG of any saved card at `isolated` or `embedded` format** (Puppeteer-driven through the prerender pool) and returns a durable served media-cache URL; nothing is written into a realm and only read access is needed. Use for **point-in-time** snapshots you keep a URL to: documentation, audit/before-after trails. The same pipeline exports a **paged PDF** — compose a durable `_capture/…?type=pdf` URL (add `media=print` for the card's `@page`/print CSS) for a printable, always-current document (invoice/report/statement), and render it through the host's `SignedCaptureLink`/`SignedCapture` components so private-realm downloads and `<object>` embeds authorize; the tool's input itself is PNG-only. For a card that should *always* carry a current picture of itself (thumbnails, og images), use `automate-declared-captures` instead.
