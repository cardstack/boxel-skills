...
- `integrate-screenshot-card-format` — `ScreenshotCardCommand` captures a **settled PNG — or a paged PDF (`type: 'pdf'`) — of any saved card at `isolated` or `embedded` format** (Puppeteer-driven through the prerender pool) and saves it under `Screenshots/` in the target card's own realm. Use for **point-in-time** snapshots kept as separate files: documentation, audit/before-after trails, or a printable **PDF export** (invoice/report/statement, with `media: 'print'` for the card's `@page`/print CSS). To **embed an always-current PDF** in a card, compose a durable `_screenshot/…?type=pdf` URL instead of storing bytes. For a card that should *always* carry a current picture of itself (thumbnails, og images), use `automate-declared-screenshots` instead.
