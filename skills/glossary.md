...
- **`PrerenderedCardSearch`** — Live-updating component that renders matching cards in a chosen format. Pass `@query`, `@realms`, `@format`, optional `@isLive`. → `show-card-list-with-views`, `app-card-home-with-prerendered-search`
...
- **two-element `.cq` → `.fit` pattern** — The mandatory fitted template skeleton. → `boxel/references/container-query-fitted-layout.md`
...
- **`/_search-prerendered`** — Realm-server endpoint returning prerendered card HTML for the 16 fitted sizes (used by host preview).
...
- **`boxel-patterns`** — Outcome-indexed catalogue of 40+ working patterns + `integration-surfaces.md` (capability cheatsheet) + `libraries.md` (import-path catalogue) + `ai-image-models.md` (verified model IDs for image generation).
...
- **`boxel-create-edit-cards`** — Command-path selection for card creation/editing.
...
- **SEARCH/REPLACE for `.gts` edits.** Never `write-text-file` for `.gts` (UI freezes).
