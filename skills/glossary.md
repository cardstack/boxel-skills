...
- **`catalog-reuse`** — Mandatory catalog search before building anything reusable. A `Spec` is the searchable *pointer*, not the card/field/component — one narrowed `Spec` query per enumerated need (`specType: card|field|component|command|app` + `matches`/`contains`, category-wide sweep before declaring a gap), then reference the definition a Spec names (its `ref`) as-is by default → remix its Listing only to modify → build new only when nothing matches (record the gap); plus file/theme instance reuse. The general form of `boxel-ui-component-discovery`.
