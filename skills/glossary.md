...
- **`getRelationshipMembershipState(this, 'field')`** — Live `{ isLoading, isLoaded, membership, totalMatchCount, isPartial }` for a `linksTo`/`linksToMany`; bind `.isLoading` to drive a spinner (flagship: query-backed `linksToMany`). Observe-only — the template must also read the field or the load never starts. `isLoaded` means settled, which a truncated set also is; `isPartial` is what licenses a reduction over the rows. → `boxel/references/relationship-loading-state.md`, `query-backed-relationships/SKILL.md`
