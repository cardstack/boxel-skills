...
- **bxl** — The workspace's unified computation runtime: a jq-flavored JSON query language + Excel-compatible formula libraries (Bessel, statistical, financial, engineering, validation). Exports `evaluateBxl`, `bxl`, `expression`, `expr`, `jq`, `fx`, `prepareBxl`, plus the formula function catalog. Card code imports it as the platform module `@cardstack/bxl`; realms that carry their own uploaded bundle import that bundle by relative path. → `bxl-authoring`, [bxl.boxel.site](https://bxl.boxel.site)
