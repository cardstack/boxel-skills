...
- **`catalog-reuse`** — Mandatory catalog search before building anything reusable: one narrowed `Spec` query per enumerated need (`specType` + `matches`/`contains`, category-wide sweep before declaring a gap) plus apps-via-Listing and file/theme instance reuse; reference as-is by default → remix only to modify → build new only when nothing matches (record the gap). The general form of `boxel-ui-component-discovery`.
