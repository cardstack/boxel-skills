...
- **`:deep()`** — Pierce the scoped-CSS boundary to reach host-generated DOM you neither render nor can address with a `class` (e.g. the embedded MarkdownDef shell body). Not for card chrome: `.boxel-card-container` and `.plural-field` both take a `class` from the field invocation. → `boxel-ui-guidelines/references/delegated-render-control.md`
...
- **`integrate-screenshot-card-format`** — Real PNG **or paged PDF** capture of a rendered card via Puppeteer; includes the durable `_screenshot/…?type=pdf` embed URL and the `media=screen|print` choice.
