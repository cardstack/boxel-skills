...
- **operation** — A named action on a card declared as plain data with `@operation`, carried out by the realm from its stored definition rather than by card JavaScript. → `card-operations-authoring`
- **base operation** — One of the nine built-in behaviors an operation builds on (`read`, `readSource`, `create`, `update`, `delete`, `query`, `transform`, `appendContainsMany`, `appendLine`). A declaration names one as its `base`; the operation's own name is what a caller invokes.
- **typed reference** — A marker standing for a value known only at invocation: `params('key')`, `actor()`, `instance()`, `realmConfig('key')`, `card(…)`. Written as calls, not interpolation.
- **lowering** — Turning an `@operation` declaration into base-operation data plus BXL when the module is indexed. Problems are recorded as findings rather than thrown, and an operation carrying one has no runnable form.
- **atomic batch** — Several operations committed all-or-nothing in one realm via `operations(card).atomic((b) => …)`, where a create's handle is the local id a later entry links the new card by.
...
- **`computeVia: expression(...)`** — bxl-flavored `computeVia` that runs a bxl expression with the card's fields as context. Validated against the `derive` profile when the field is defined, so a volatile or request-scoped expression throws as the card module loads. → `bxl-authoring`
...
- **card instance id** — The URL of a card instance with no extension (`https://realm/FacebookProfile/maria-santos`). The `.json` file at that URL plus the extension stores the instance; a `.gts` URL is a definition module and is not a card id. Every `cardId` tool argument takes the instance id. → `boxel-environment/references/host-commands-reference.md`
...
- **`static captures`** — Declared self-refreshing capture slots on a CardDef/FileDef: `static captures: Record<string, CaptureSpec> = {…}` (the annotation is required — without it TS widens the literals and the class fails TS2417). Each `CaptureSpec` declares exactly one of `format` or `render` (a capture-only component), required `width`/`height`, and optional `useAsThumbnail` (at most one slot; feeds `cardThumbnailURL`) and `keyBy: 'generation' | 'file-content'` (what invalidates the capture). Captured server-side on every index. → `automate-declared-captures`
- **`captureURLs`** — Reserved getter on every CardDef/FileDef (an `@field` may not use the name): one key per declared capture slot, `undefined` until a capture exists — always guard the `<img>` and build fallback chains on `undefined`. → `automate-declared-captures`
- **`cardThumbnailURL`** — Computed the default fitted tile renders; fallback chain: author-set URL → authored `cardInfo.cardThumbnail` link → the `useAsThumbnail` capture. → `automate-declared-captures`
- **`data-capture-pending`** — DOM attribute a capture-only component renders while async content (video frame, PDF page, WebGL first frame) is unready; the capture engine waits (bounded) until no such element remains before shooting. → `automate-declared-captures`
...
- **Theme CardDef** — DO NOT USE. The bare `Theme` in `@cardstack/base/card-api` is a root class whose free-form `cssVariables` string bypasses the token contract: never instantiate or subclass it. New themes start from `StructuredTheme` at minimum. → `boxel/references/theme-design-system.md` (ThemeCard Types)
...
- **custom theme variables** — Tokens outside the contract. `StructuredTheme` has no slot for them; they come from a `BrandGuide` (`customCssVariables`, `brandColorPalette`) or an extended theme card definition, and give up the contract's default and boundary reset. → `boxel-ui-guidelines/references/theme-token-contract.md` ("Not part of the contract")
...
- **`LogFile` / `JSONLFile`** — The `TextFileDef` subclasses every stored `.log` / `.jsonl` file is. Each declares a named `record` operation built on `appendLine`, whose line (or JSON entry) the realm stamps with the time and the authenticated caller. → `card-operations-authoring`
...
- **bxl** — The workspace's unified computation runtime: a jq-flavored JSON query language + Excel-compatible formula libraries (Bessel, statistical, financial, engineering, validation). Exports `evaluateBxl`, `bxl`, `expression`, `expr`, `jq`, `fx`, `prepareBxl`, plus the formula function catalog. **Import it from `@cardstack/bxl`** — it is not realm-bundled and needs nothing hosted in the workspace. Listed in this section only for the legacy case: a realm that carries its own uploaded bundle imports that bundle by relative path (`from '../bxl'`), which is how older realms still reach it. → `bxl-authoring`, [bxl.boxel.site](https://bxl.boxel.site)
...
- **`CaptureCardTool`** — Captures a settled PNG of any saved card at `isolated` or `embedded` format (Puppeteer-driven via prerender pool) and returns a durable served media-cache URL (`captures[0].url`); needs realm read only, writes no realm file. **Point-in-time** snapshots; for a card that should *always* carry a current picture of itself (thumbnails, og images), declare `static captures` instead. → `integrate-capture-card-format`, `automate-declared-captures`
- **`type: 'pdf'`** — Capture-spec output encoding that paginates the settled render into a PDF document instead of a raster (default `type: 'png'`). Lives on the capture-spec surfaces (`_capture/` URL, `POST /_capture-card`), not on `CaptureCardTool`'s input. Bounded post-render at **20 pages / 10 MB** — over either is a capture error naming the cap, never a truncation. Singular-only; incompatible with `fullPage`/`clip`/`target`/`viewport`. → `integrate-capture-card-format`
- **`media: 'print'`** — Capture-spec axis choosing the CSS media the render settles under (default `media: 'screen'`). `print` engages the card's `@page` (paper size), `@media print`, and `break-*` CSS — the paper-layout choice, most relevant to `type: 'pdf'`. No `@page { size }` rule ⇒ Chrome's Letter default. → `integrate-capture-card-format`
- **durable PDF URL** — A `{realm}_capture/{card-path}?type=pdf[&media=print]` serving URL embedded in a card: served from MediaCache (ledger hit on repeat, capture on miss), re-captured when the source card is edited, so an embedded "download PDF" link is always current; served with `Content-Disposition: inline` and a filename derived from the card path. Prefer over storing one-off base64 bytes. Served behind realm read — on a private realm render it through `SignedCaptureLink` / `SignedCapture`, never a bare anchor or `<object>`. → `integrate-capture-card-format`
- **signed capture URL** — A durable `_capture/` URL plus a short-lived `?token=` (15 min, bound to one realm, one URL, one user; `read-capture` scope) that authorizes that single GET without an `Authorization` header — for the loads the auth service worker cannot reach: `<object>`/`<embed>` embeds and new-tab navigations, which otherwise 401 on a private realm. Minted by `QUERY {realm}_sign-capture-urls` for callers who already hold realm read (public realms echo the URL unsigned). **Ephemeral view-layer value — never persist it**; the durable URL is the only storable reference. → `integrate-capture-card-format`
- **`SignedCaptureLink` / `SignedCapture`** — Card-facing components from `@cardstack/boxel-host/lib/signed-capture` that hide signed-capture minting. `SignedCaptureLink` is an anchor whose `href` stays the durable URL and whose click opens a new tab with a fresh token (popup-blocker-safe). `SignedCapture` is a renderless provider yielding `[signedUrl, error]` for render-time attributes (`<object data>`, `<embed src>`); yields nothing during prerender. → `integrate-capture-card-format`
...
- **`rich-markdown-reports`** — For reports/summaries/dashboards, compose a Rich Markdown `.md` that embeds existing cards (custom or off-the-shelf) instead of authoring a bespoke CardDef to present it.
...
- **`automate-declared-captures`** — Self-refreshing capture slots via `static captures`, consumed as `@model.captureURLs.<name>`; a `useAsThumbnail: true` slot feeds `cardThumbnailURL` so grid tiles show the real rendering with zero template edits. Prefer over the imperative capture/thumbnail commands whenever the card should *always* have a current picture of itself.
...
- **`integrate-capture-card-format`** — Real PNG **or paged PDF** capture of a rendered card via Puppeteer; includes the durable `_capture/…?type=pdf` embed URL, the `media=screen|print` choice, and the `SignedCaptureLink`/`SignedCapture` components that make private-realm downloads and `<object>` embeds authorize.
