...
- **🚨 Base modules import by URL.** `https://cardstack.com/base/<module>` (`card-api`, `string`, `date`, ...), never `@cardstack/base/...` — that package name does not resolve in a realm. The `@cardstack/*` scope is only for `@cardstack/boxel-ui/*`, `@cardstack/boxel-icons/*`, `@cardstack/runtime-common`. → `boxel/references/common-imports.md`
