...
- **`catalog-reuse`** — Mandatory catalog search before building anything reusable: `Spec` search (`specType: card|field|component|command`) plus apps-via-Listing and file/theme instance reuse; reference as-is by default → remix only to modify → build new only when nothing matches (record the gap). The general form of `boxel-ui-component-discovery`.
