...
```
**Note:** `codePath` pins code mode to the target realm — a bare `submode` switch stays in whatever realm the UI last showed. It is a plain file URL: the `(new)` marker belongs after the URL in a `run-realm-code` tool call, never inside a tool argument.
