...
A component may legitimately opt out of the theme: a capture keyed on file content must render the same for every viewer, a print sheet must not follow a dark theme. When that is the case and there is a real reason not to use the fixed `--boxel-*` primitives from boxel-ui's `variables.css`, declare the constants under the component's own prefix (`--poster-*`), never by reassigning a name the contract owns.

Writing `--card: #fff` or `--border: #d8d8d8` on a root is not a private constant; it is a theme override. Every descendant and every child card rendered inside inherits the pinned value, a linked Brand Guide or Theme card is silently ignored for the subtree, and a reader (or a hex grep) sees `var(--card)` below and assumes the contract is being honored. `--poster-paper: #fff` carries the same value with none of that: nothing outside the component reads it, and its name says it is not themed.
...
- **Contract tokens reassigned.** `--card`, `--card-foreground`, `--muted`, `--muted-foreground`, `--border`, `--tooltip`, `--tooltip-foreground`, `--shadow-sm`, and `--font-sans` are overwritten. The `var(--card)` / `var(--border)` reads below look themed and are not, and any child card inside `.office-poster` inherits the pinned values.
...
**Right** — the same opt-out, owned end to end by the component's prefix, in one unit:
```css
.office-poster {
  --poster-paper: #fff;
  --poster-ink: #1a1a1a;
  --poster-ink-muted: #555;
  --poster-rule: #d8d8d8;
  --poster-font: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;

  --poster-title-size: 13px;
  --poster-heading-size: 11px;
  --poster-body-size: 9px;
  --poster-cell-size: 8px;
  --poster-page-padding: 14px 12px;
  --poster-block-gap: 5px;
  --poster-radius: 3px;
...
Nothing in the Right version can be mistaken for a themed token, nothing leaks into a child card, and the one unit keeps the proportions fixed at every root font size.
