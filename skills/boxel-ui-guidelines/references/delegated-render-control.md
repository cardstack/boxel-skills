...
**Related but distinct concern — Host-mode click-through.** When you render child cards via `PrerenderedCardSearch` inside an app card that publishes to Host mode, the rendered tiles need an explicit `<a href={{c.url}}>` overlay to be clickable. The in-app `@context.cardComponentModifier` machinery only fires in Interact / Code mode. See [`app-card-home-with-prerendered-search`](../../boxel-patterns/patterns/app-card-home-with-prerendered-search/README.md) → "Host-mode click-through" for the overlay pattern.
