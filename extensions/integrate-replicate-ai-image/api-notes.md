...
// Persist result as a realm file before linking it from a card
let base64Content = imageUrl.startsWith('data:')
  ? imageUrl.slice(imageUrl.indexOf(',') + 1)
  : await fetch(imageUrl)
      .then((response) => response.arrayBuffer())
      .then((buffer) => btoa(String.fromCharCode(...new Uint8Array(buffer))));

const written = await new WriteBinaryFileCommand(ctx).execute({
...
return new ImageDef({
  id: written.fileIdentifier,
  sourceUrl: written.fileIdentifier,
  url: written.fileIdentifier,
  name: 'result.png',
  contentType: 'image/png',
});
```
