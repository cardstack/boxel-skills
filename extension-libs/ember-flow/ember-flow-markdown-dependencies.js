...
let elkConstructorPromise;

async function getElkConstructor() {
  if (!elkConstructorPromise) {
    elkConstructorPromise = (async () => {
      let response = await fetch(new URL('./ember-flow-elk-bundled.txt', import.meta.url));

      if (!response.ok) {
        throw new Error(`Unable to load ELK runtime: HTTP ${response.status}`);
      }

      let previousElk = globalThis.ELK;
      let source = await response.text();

      new Function(`${source}\n//# sourceURL=ember-flow-elk-bundled.txt`)();

      let constructor = globalThis.ELK?.default ?? globalThis.ELK;

      if (previousElk === undefined) {
        delete globalThis.ELK;
      } else {
        globalThis.ELK = previousElk;
      }

      if (!constructor) {
        throw new Error('ELK runtime did not expose a constructor.');
      }
...
function defaultNodeWidth(node) {
          ...
          function filterEdgesForElkLayout(edges, nodes, eligibleIds, edgeMode) {
            ...
            return edges.filter((edge) => {
              ...
              if (edgeMode === 'visible' && edge.hidden) {
                return false;
              }

              if (edgeMode === 'external' && nodeById.get(edge.source)?.parentId === nodeById.get(edge.target)?.parentId) {
                return false;
              }
          ...
          function buildCompoundElkChildren(nodes, eligibleIds, getNodeWidth, getNodeHeight, groupPadding) {
            let childrenByParent = new Map();

            for (let node of nodes) {
              if (!node.parentId || !eligibleIds.has(node.parentId)) {
                continue;
              }

              let children = childrenByParent.get(node.parentId) ?? [];
              children.push(node);
            ...
            return nodes.filter((node) => !node.parentId || !eligibleIds.has(node.parentId)).map(toElkNode);
          }

          function collectElkPositions(children, positions, dimensions, origin, hierarchy) {
            for (let child of children) {
              ...
              if (child.width !== undefined && child.height !== undefined) {
                dimensions.set(child.id, { width: child.width, height: child.height });
              }

              if (child.children?.length) {
                collectChildElkPositions(child.children, positions, dimensions, hierarchy);
              }
            }
          }

          function collectChildElkPositions(children, positions, dimensions, hierarchy) {
            for (let child of children) {
              positions.set(child.id, {
                x: child.x ?? 0,
                y: child.y ?? 0,
          ...
          async function applyElkLayout({
            ...
            options = {},
            elkOptions = {},
            graphId = 'ember-flow-elk-layout',
            hierarchy = 'flat',
            edgeMode = 'all',
            groupPadding = 32,
            elk,
          }) {
            let Elk = elk ? null : await getElkConstructor();
            let layoutEngine = elk ?? new Elk();

            if (!layoutEngine?.layout) {
              throw new Error('ELK layout engine is unavailable. Expected ember-flow-elk-bundled.txt to load an elkjs constructor.');
            }
            ...
            let eligibleEdges = filterEdgesForElkLayout(edges, eligibleNodes, eligibleIds, edgeMode);
            let graph = await layoutEngine.layout({
              id: graphId,
              layoutOptions: {
                'elk.algorithm': 'layered',
                'elk.direction': elkDirection(direction),
                'elk.spacing.nodeNode': String(options.nodeSpacing ?? 72),
                'elk.layered.spacing.nodeNodeBetweenLayers': String(options.rankSpacing ?? 140),
                ...(hierarchy === 'parentId' ? { 'elk.hierarchyHandling': 'INCLUDE_CHILDREN' } : {}),
                ...elkOptions,
              },
              children:
                hierarchy === 'parentId'
                  ? buildCompoundElkChildren(eligibleNodes, eligibleIds, getNodeWidth, getNodeHeight, groupPadding)
                  : eligibleNodes.map((node) => ({
                      id: node.id,
                      width: getNodeWidth(node),
                      height: getNodeHeight(node),
                    })),
              edges: eligibleEdges.map((edge) => ({
                id: edge.id,
                sources: [edge.source],
                targets: [edge.target],
              })),
            ...
            collectElkPositions(graph.children ?? [], positions, dimensions, origin, hierarchy);

            return {
              nodes: nodes.map((node) => {
                let position = positions.get(node.id);
                let dimension = dimensions.get(node.id);
