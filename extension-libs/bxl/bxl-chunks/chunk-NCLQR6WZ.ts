...
var require_isBase64 = __commonJS({
  "node_modules/validator/lib/isBase64.js"(exports, module) {
    ...
    function isBase64(str, options) {
      ...
      if (str === "")
        return true;
      if (options.padding && str.length % 4 !== 0)
        return false;
      var regex;
      if (options.urlSafe) {
    ...
    function ptBrCheck(tin) {
      ...
      if (tin.length === 11) {
        ...
        if (remainder !== parseInt(tin.substring(9, 10), 10))
          return false;
        sum = 0;
        for (var _i9 = 1; _i9 <= 10; _i9++)
          sum += parseInt(tin.substring(_i9 - 1, _i9), 10) * (12 - _i9);
        remainder = sum * 10 % 11;
        if (remainder === 10)
          remainder = 0;
...
// src/bxl/bridge/validation-native.ts
var import_validator = __toESM(require_validator(), 1);

// src/bxl/bridge/validation-manifest.ts
var VALIDATION_FUNCTION_DEFINITIONS = [
...
var VALIDATION_FUNCTIONS = VALIDATION_FUNCTION_DEFINITIONS.map(
  (definition) => definition.name
);
var DETERMINISTIC_VALIDATION_FUNCTIONS = VALIDATION_FUNCTION_DEFINITIONS.filter((definition) => !("volatile" in definition && definition.volatile)).map((definition) => definition.name);
var VOLATILE_VALIDATION_FUNCTIONS = VALIDATION_FUNCTION_DEFINITIONS.filter((definition) => "volatile" in definition && definition.volatile).map((definition) => definition.name);
var VALIDATION_FILTERS = new Set(
  VALIDATION_FUNCTION_DEFINITIONS.flatMap(
    (definition) => definition.arities.map((arity) => `${definition.name}/${arity}`)
  )
);
var VALIDATION_DEFINITION_BY_LOWERCASE = new Map(
  VALIDATION_FUNCTION_DEFINITIONS.map(
    (definition) => [definition.name.toLowerCase(), definition]
  )
);
function validationFunctionDefinition(name) {
  return VALIDATION_DEFINITION_BY_LOWERCASE.get(name.toLowerCase());
}
function canonicalValidationFunctionName(name, explicitArity) {
  const definition = validationFunctionDefinition(name);
  if (!definition) {
    return void 0;
  }
  if (explicitArity !== void 0 && !definition.arities.includes(explicitArity)) {
    return void 0;
  }
...
function callOpenIndex(source, index) {
  let cursor = index;
...
function callArity(source, openIndex) {
  let depth = 0;
  let inString = false;
  let escaped = false;
...
function sourceUsesValidationFunction(source) {
  let inString = false;
  let escaped = false;
  let inComment = false;
  for (let index = 0; index < source.length; index++) {
    ...
    const arity = openIndex >= 0 ? callArity(source, openIndex) : void 0;
    if (openIndex >= 0 && arity !== void 0 && canonicalValidationFunctionName(match[0], arity) && !isIdentifierChar(source[index + match[0].length])) {
      return true;
    }
...
// src/bxl/bridge/validation-native.ts
var validator = import_validator.default.default ?? import_validator.default;
function validatorFunction(name) {
  const candidate = validator[name];
  return typeof candidate === "function" ? candidate : void 0;
}
function callStringValidator(name, value, ...args) {
...
for (const definition of VALIDATION_FUNCTION_DEFINITIONS) {
  for (const arity of definition.arities) {
    bareNativeFilters[`${definition.name}/${arity}`] = function* (_input, value, ...args) {
      yield callStringValidator(definition.name, value, ...args);
    };
  }
}
var validationNativeFilters = wrapBareNativeFilters(bareNativeFilters);

// src/bxl/registry/validation.ts
