  ...
  "node_modules/jstat/dist/jstat.js"(exports, module) {
    (function(window, factory) {
      ...
      (function(jStat3, Math2) {
        ...
        jStat3.factorial = function factorial2(n) {
      ...
      (function(jStat3, Math2) {
        ...
        jStat3.extend(jStat3.normal, {
          ...
          cdf: function cdf(x, mean, std) {
            return 0.5 * (1 + jStat3.erf((x - mean) / Math2.sqrt(2 * std * std)));
          },
          inv: function(p, mean, std) {
            return -1.4142135623730951 * std * jStat3.erfcinv(2 * p) + mean;
          },
          mean: function(mean) {
            return mean;
          },
      ...
      (function(jStat3, Math2) {
        ...
        jStat3.extend({
          ...
          cubic_spline: function cubic_spline(X, F, value) {
            ...
            j -= 1;
            return F[j] + (value - X[j]) * b[j] + jStat3.sq(value - X[j]) * c[j] + (value - X[j]) * jStat3.sq(value - X[j]) * d[j];
          },
...
// src/formulajs/statistical.ts
var import_jstat = __toESM(require_jstat(), 1);
var jStat = import_jstat.default;
...
function numericMatrix(value) {
  ...
  const rows = value.map(
    (row) => Array.isArray(row) ? row.map((entry) => parseExcelNumber(entry)) : [parseExcelNumber(row)]
  );
...
function excelBetaDist(xLike, alphaLike, betaLike, cumulativeLike, aLike = 0, bLike = 1) {
...
function excelBetaInv(probabilityLike, alphaLike, betaLike, aLike = 0, bLike = 1) {
...
function excelBinomDist(numberSLike, trialsLike, probabilitySLike, cumulativeLike) {
  ...
  return checkedNumber(
    cumulative ? jStat.binomial.cdf(numberS, trials, probabilityS) : jStat.binomial.pdf(numberS, trials, probabilityS)
  );
}
function excelBinomDistRange(trialsLike, probabilitySLike, numberSLike, numberS2Like = numberSLike) {
  const trials = Math.floor(parseExcelNumber(trialsLike));
  const probabilityS = parseExcelNumber(probabilitySLike);
  const numberS = Math.floor(parseExcelNumber(numberSLike));
  const numberS2 = Math.floor(parseExcelNumber(numberS2Like));
  if (trials < 0 || probabilityS < 0 || probabilityS > 1 || numberS < 0 || numberS2 < numberS || numberS2 > trials) {
    throwExcelError(EXCEL_ERROR.num);
  }
  let result = 0;
...
function excelBinomInv(trialsLike, probabilitySLike, alphaLike) {
  const trials = Math.floor(parseExcelNumber(trialsLike));
  const probabilityS = parseExcelNumber(probabilitySLike);
  const alpha = parseExcelNumber(alphaLike);
...
function excelChisqDist(xLike, degFreedomLike, cumulativeLike) {
  const x = parseExcelNumber(xLike);
  ...
  return checkedNumber(
    cumulative ? jStat.chisquare.cdf(x, degFreedom) : jStat.chisquare.pdf(x, degFreedom)
  );
}
function excelChisqDistRt(xLike, degFreedomLike) {
  const x = parseExcelNumber(xLike);
  const degFreedom = parseExcelNumber(degFreedomLike);
  if (x < 0 || degFreedom < 1 || degFreedom > 1e10) {
    throwExcelError(EXCEL_ERROR.num);
  }
  return checkedNumber(1 - jStat.chisquare.cdf(x, degFreedom));
}
function excelChisqInv(probabilityLike, degFreedomLike) {
  const probability = parseExcelNumber(probabilityLike);
  const degFreedom = parseExcelNumber(degFreedomLike);
...
function excelChisqInvRt(probabilityLike, degFreedomLike) {
  const probability = parseExcelNumber(probabilityLike);
  const degFreedom = parseExcelNumber(degFreedomLike);
  if (probability < 0 || probability > 1 || degFreedom < 1 || degFreedom > 1e10) {
    throwExcelError(EXCEL_ERROR.num);
  }
  return checkedNumber(jStat.chisquare.inv(1 - probability, degFreedom));
}
function excelChisqTest(actualLike, expectedLike) {
...
function excelConfidenceNorm(alphaLike, standardDevLike, sizeLike) {
  const alpha = parseExcelNumber(alphaLike);
...
function excelConfidenceT(alphaLike, standardDevLike, sizeLike) {
  const alpha = parseExcelNumber(alphaLike);
  const standardDev = parseExcelNumber(standardDevLike);
  const size = parseExcelNumber(sizeLike);
  if (alpha <= 0 || alpha >= 1 || standardDev <= 0 || size < 1) {
...
function excelExponDist(xLike, lambdaLike, cumulativeLike) {
  const x = parseExcelNumber(xLike);
  ...
  return checkedNumber(
    cumulative ? jStat.exponential.cdf(x, lambda) : jStat.exponential.pdf(x, lambda)
  );
}
function excelFDist(xLike, degFreedom1Like, degFreedom2Like, cumulativeLike) {
  const x = parseExcelNumber(xLike);
  ...
  return checkedNumber(
    cumulative ? jStat.centralF.cdf(x, degFreedom1, degFreedom2) : jStat.centralF.pdf(x, degFreedom1, degFreedom2)
  );
}
function excelFDistRt(xLike, degFreedom1Like, degFreedom2Like) {
  const x = parseExcelNumber(xLike);
  const degFreedom1 = parseExcelNumber(degFreedom1Like);
  const degFreedom2 = parseExcelNumber(degFreedom2Like);
  if (x < 0 || degFreedom1 < 1 || degFreedom2 < 1) {
...
function excelFInv(probabilityLike, degFreedom1Like, degFreedom2Like) {
  const probability = parseExcelNumber(probabilityLike);
  const degFreedom1 = parseExcelNumber(degFreedom1Like);
  const degFreedom2 = parseExcelNumber(degFreedom2Like);
...
function excelFInvRt(probabilityLike, degFreedom1Like, degFreedom2Like) {
  const probability = parseExcelNumber(probabilityLike);
  const degFreedom1 = parseExcelNumber(degFreedom1Like);
  const degFreedom2 = parseExcelNumber(degFreedom2Like);
  if (probability < 0 || probability > 1 || degFreedom1 < 1 || degFreedom2 < 1) {
...
function excelFTest(array1Like, array2Like) {
...
function excelGammaDist(valueLike, alphaLike, betaLike, cumulativeLike) {
  const value = parseExcelNumber(valueLike);
  const alpha = parseExcelNumber(alphaLike);
  const beta = parseExcelNumber(betaLike);
  const cumulative = parseExcelBool(cumulativeLike);
...
function excelGammaInv(probabilityLike, alphaLike, betaLike) {
  const probability = parseExcelNumber(probabilityLike);
  const alpha = parseExcelNumber(alphaLike);
  const beta = parseExcelNumber(betaLike);
...
function excelGammaln(valueLike) {
  const value = parseExcelNumber(valueLike);
  if (value <= 0) {
    throwExcelError(EXCEL_ERROR.num);
  }
...
function excelGauss(zLike) {
  const z = parseExcelNumber(zLike);
  return checkedNumber(jStat.normal.cdf(z, 0, 1) - 0.5);
}
function excelHypgeomDist(sampleSLike, numberSampleLike, populationSLike, numberPopLike, cumulativeLike) {
  ...
  const cumulative = parseExcelBool(cumulativeLike);
  if (sampleS < 0 || numberSample < 0 || populationS < 0 || numberPop < 0 || sampleS > populationS || numberSample > numberPop || populationS > numberPop) {
    throwExcelError(EXCEL_ERROR.num);
  }
  if (cumulative) {
    let result = 0;
...
function excelLognormDist(xLike, meanLike, standardDevLike, cumulativeLike) {
  const x = parseExcelNumber(xLike);
  ...
  return checkedNumber(
    cumulative ? jStat.lognormal.cdf(x, mean, standardDev) : jStat.lognormal.pdf(x, mean, standardDev)
  );
}
function excelLognormInv(probabilityLike, meanLike, standardDevLike) {
  const probability = parseExcelNumber(probabilityLike);
  const mean = parseExcelNumber(meanLike);
  const standardDev = parseExcelNumber(standardDevLike);
...
function excelNegbinomDist(numberFLike, numberSLike, probabilitySLike, cumulativeLike) {
...
function excelNormDist(xLike, meanLike, standardDevLike, cumulativeLike) {
  const x = parseExcelNumber(xLike);
  const mean = parseExcelNumber(meanLike);
  const standardDev = parseExcelNumber(standardDevLike);
  const cumulative = parseExcelBool(cumulativeLike);
...
function excelNormInv(probabilityLike, meanLike, standardDevLike) {
  const probability = parseExcelNumber(probabilityLike);
  const mean = parseExcelNumber(meanLike);
  const standardDev = parseExcelNumber(standardDevLike);
  if (probability < 0 || probability > 1 || standardDev <= 0) {
...
function excelNormSDist(zLike, cumulativeLike) {
  return excelNormDist(zLike, 0, 1, cumulativeLike);
}
function excelNormSInv(probabilityLike) {
  return excelNormInv(probabilityLike, 0, 1);
}
function excelPhi(xLike) {
  const x = parseExcelNumber(xLike);
  return checkedNumber(Math.exp(-0.5 * x ** 2) / Math.sqrt(2 * Math.PI));
}
function excelPoissonDist(xLike, meanLike, cumulativeLike) {
...
function excelStandardize(xLike, meanLike, standardDevLike) {
  const x = parseExcelNumber(xLike);
  const mean = parseExcelNumber(meanLike);
  const standardDev = parseExcelNumber(standardDevLike);
  if (standardDev <= 0) {
...
function excelTDist(xLike, degFreedomLike, cumulativeLike) {
  const x = parseExcelNumber(xLike);
  const degFreedom = parseExcelNumber(degFreedomLike);
  const cumulative = parseExcelBool(cumulativeLike);
  if (degFreedom < 1) {
...
function excelTDist2T(xLike, degFreedomLike) {
  const x = parseExcelNumber(xLike);
  const degFreedom = parseExcelNumber(degFreedomLike);
  if (x < 0 || degFreedom < 1) {
    throwExcelError(EXCEL_ERROR.num);
...
function excelTDistRt(xLike, degFreedomLike) {
  const x = parseExcelNumber(xLike);
  const degFreedom = parseExcelNumber(degFreedomLike);
  if (x < 0 || degFreedom < 1) {
    throwExcelError(EXCEL_ERROR.num);
...
function excelTInv(probabilityLike, degFreedomLike) {
  const probability = parseExcelNumber(probabilityLike);
  const degFreedom = parseExcelNumber(degFreedomLike);
  if (probability < 0 || probability > 1 || degFreedom < 1) {
    throwExcelError(EXCEL_ERROR.num);
...
function excelTInv2T(probabilityLike, degFreedomLike) {
  const probability = parseExcelNumber(probabilityLike);
  const degFreedom = parseExcelNumber(degFreedomLike);
  if (probability <= 0 || probability > 1 || degFreedom < 1) {
    throwExcelError(EXCEL_ERROR.num);
...
function excelTTest(array1Like, array2Like) {
  const array1 = finiteNumbers(array1Like);
  const array2 = finiteNumbers(array2Like);
  if (array1.length < 2 || array2.length < 2) {
    throwExcelError(EXCEL_ERROR.div0);
...
function excelWeibullDist(xLike, alphaLike, betaLike, cumulativeLike) {
  const x = parseExcelNumber(xLike);
  const alpha = parseExcelNumber(alphaLike);
  const beta = parseExcelNumber(betaLike);
  const cumulative = parseExcelBool(cumulativeLike);
...
function excelZTest(arrayLike, xLike, sigmaLike) {
  const array = finiteNumbers(flattenExcelArgs(arrayLike));
  if (array.length === 0) {
    throwExcelError(EXCEL_ERROR.na);
  }
  const x = parseExcelNumber(xLike);
  const sigma = sigmaLike === void 0 ? Math.sqrt(jStat.variance(array, true)) : parseExcelNumber(sigmaLike);
  if (sigma <= 0) {
    throwExcelError(EXCEL_ERROR.num);
  }
...
// src/bxl/bridge/formula-statistical-native.ts
var bareNativeFilters = {
  *"BETA_DIST/4"(_input, x, alpha, beta, cumulative) {
    yield excelBetaDist(x, alpha, beta, cumulative);
  },
  *"BETA_DIST/5"(_input, x, alpha, beta, cumulative, a) {
    yield excelBetaDist(x, alpha, beta, cumulative, a);
  },
  *"BETA_DIST/6"(_input, x, alpha, beta, cumulative, a, b) {
    yield excelBetaDist(x, alpha, beta, cumulative, a, b);
  },
  *"BETA_INV/3"(_input, probability, alpha, beta) {
    yield excelBetaInv(probability, alpha, beta);
  },
  *"BETA_INV/4"(_input, probability, alpha, beta, a) {
    yield excelBetaInv(probability, alpha, beta, a);
  },
  *"BETA_INV/5"(_input, probability, alpha, beta, a, b) {
    yield excelBetaInv(probability, alpha, beta, a, b);
  },
  *"BINOM_DIST/4"(_input, numberS, trials, probabilityS, cumulative) {
    yield excelBinomDist(numberS, trials, probabilityS, cumulative);
  },
  *"BINOM_DIST_RANGE/3"(_input, trials, probabilityS, numberS) {
    yield excelBinomDistRange(trials, probabilityS, numberS);
  },
  *"BINOM_DIST_RANGE/4"(_input, trials, probabilityS, numberS, numberS2) {
    yield excelBinomDistRange(trials, probabilityS, numberS, numberS2);
  },
  *"BINOM_INV/3"(_input, trials, probabilityS, alpha) {
    yield excelBinomInv(trials, probabilityS, alpha);
  },
  *"CHISQ_DIST/3"(_input, x, degFreedom, cumulative) {
    yield excelChisqDist(x, degFreedom, cumulative);
  },
  *"CHISQ_DIST_RT/2"(_input, x, degFreedom) {
    yield excelChisqDistRt(x, degFreedom);
  },
  *"CHISQ_INV/2"(_input, probability, degFreedom) {
    yield excelChisqInv(probability, degFreedom);
  },
  *"CHISQ_INV_RT/2"(_input, probability, degFreedom) {
    yield excelChisqInvRt(probability, degFreedom);
  },
  *"CHISQ_TEST/2"(_input, actual, expected) {
    yield excelChisqTest(actual, expected);
  },
  *"CONFIDENCE_NORM/3"(_input, alpha, standardDev, size) {
    yield excelConfidenceNorm(alpha, standardDev, size);
  },
  *"CONFIDENCE_T/3"(_input, alpha, standardDev, size) {
    yield excelConfidenceT(alpha, standardDev, size);
  },
  *"EXPON_DIST/3"(_input, x, lambda, cumulative) {
    yield excelExponDist(x, lambda, cumulative);
  },
  *"F_DIST/4"(_input, x, degFreedom1, degFreedom2, cumulative) {
    yield excelFDist(x, degFreedom1, degFreedom2, cumulative);
  },
  *"F_DIST_RT/3"(_input, x, degFreedom1, degFreedom2) {
    yield excelFDistRt(x, degFreedom1, degFreedom2);
  },
  *"F_INV/3"(_input, probability, degFreedom1, degFreedom2) {
    yield excelFInv(probability, degFreedom1, degFreedom2);
  },
  *"F_INV_RT/3"(_input, probability, degFreedom1, degFreedom2) {
    yield excelFInvRt(probability, degFreedom1, degFreedom2);
  },
  *"F_TEST/2"(_input, array1, array2) {
    yield excelFTest(array1, array2);
  },
  *"GAMMA/1"(_input, value) {
    yield excelGamma(value);
  },
  *"GAMMA_DIST/4"(_input, value, alpha, beta, cumulative) {
    yield excelGammaDist(value, alpha, beta, cumulative);
  },
  *"GAMMA_INV/3"(_input, probability, alpha, beta) {
    yield excelGammaInv(probability, alpha, beta);
  },
  *"GAMMALN/1"(_input, value) {
    yield excelGammaln(value);
  },
  *"GAMMALN_PRECISE/1"(_input, value) {
    yield excelGammaln(value);
  },
  *"GAUSS/1"(_input, z) {
    yield excelGauss(z);
  },
  *"HYPGEOM_DIST/5"(_input, sampleS, numberSample, populationS, numberPop, cumulative) {
    yield excelHypgeomDist(sampleS, numberSample, populationS, numberPop, cumulative);
  },
  *"LOGNORM_DIST/4"(_input, x, mean, standardDev, cumulative) {
    yield excelLognormDist(x, mean, standardDev, cumulative);
  },
  *"LOGNORM_INV/3"(_input, probability, mean, standardDev) {
    yield excelLognormInv(probability, mean, standardDev);
  },
  *"NEGBINOM_DIST/4"(_input, numberF, numberS, probabilityS, cumulative) {
    yield excelNegbinomDist(numberF, numberS, probabilityS, cumulative);
  },
  *"NORM_DIST/4"(_input, x, mean, standardDev, cumulative) {
    yield excelNormDist(x, mean, standardDev, cumulative);
  },
  *"NORM_INV/3"(_input, probability, mean, standardDev) {
    yield excelNormInv(probability, mean, standardDev);
  },
  *"NORM_S_DIST/2"(_input, z, cumulative) {
    yield excelNormSDist(z, cumulative);
  },
  *"NORM_S_INV/1"(_input, probability) {
    yield excelNormSInv(probability);
  },
  *"PHI/1"(_input, x) {
    yield excelPhi(x);
  },
  *"POISSON_DIST/3"(_input, x, mean, cumulative) {
    yield excelPoissonDist(x, mean, cumulative);
  },
  *"STANDARDIZE/3"(_input, x, mean, standardDev) {
    yield excelStandardize(x, mean, standardDev);
  },
  *"T_DIST/3"(_input, x, degFreedom, cumulative) {
    yield excelTDist(x, degFreedom, cumulative);
  },
  *"T_DIST_2T/2"(_input, x, degFreedom) {
    yield excelTDist2T(x, degFreedom);
  },
  *"T_DIST_RT/2"(_input, x, degFreedom) {
    yield excelTDistRt(x, degFreedom);
  },
  *"T_INV/2"(_input, probability, degFreedom) {
    yield excelTInv(probability, degFreedom);
  },
  *"T_INV_2T/2"(_input, probability, degFreedom) {
    yield excelTInv2T(probability, degFreedom);
  },
  *"T_TEST/2"(_input, array1, array2) {
    yield excelTTest(array1, array2);
  },
  *"WEIBULL_DIST/4"(_input, x, alpha, beta, cumulative) {
    yield excelWeibullDist(x, alpha, beta, cumulative);
  },
  *"Z_TEST/2"(_input, array, x) {
    yield excelZTest(array, x);
  },
  *"Z_TEST/3"(_input, array, x, sigma) {
    yield excelZTest(array, x, sigma);
  }
};
var formulaStatisticalNativeFilters = wrapBareNativeFilters(bareNativeFilters);

// src/bxl/registry/formula-statistical.ts
var formulaStatisticalLibrary = {
  jq: {},
