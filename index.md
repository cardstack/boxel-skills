...
**Reuse before you build.** Before authoring any new card, field, component, command, app, or asset, read [`skills/catalog-reuse/SKILL.md`](skills/catalog-reuse/SKILL.md) and search the catalog first. A `Spec` is the catalog's searchable *pointer*, not the definition itself — search Specs, then reference the card/field/component a Spec names (its `ref`) as-is by default, remix its Listing only to modify, and build new only when nothing matches. The failure it prevents is hand-building what the catalog already ships (or reusing one thing and hand-rolling the rest); a search is complete only after sweeping the relevant `specType`, not on a single miss.
...
- **[`catalog-reuse/`](skills/catalog-reuse/SKILL.md)** — Mandatory catalog search before authoring any card, field, component, command, app, or asset. A `Spec` is the searchable *pointer*, not the definition — search Specs, then reference the card/field/component a Spec names (its `ref`) as-is by default, remix its Listing only to modify, build new only when nothing matches. The general form of `boxel-ui-component-discovery`.
